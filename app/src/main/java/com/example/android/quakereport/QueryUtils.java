...
/**
 * Created by Christi on 05.04.2017.
 */
...
public final class QueryUtils {
    ...
    /**
     * Query the USGS dataset and return an {@link ArrayList<Earthquake>} to represent a list of earthquakes.
     */
    public static ArrayList<Earthquake> fetchEarthquakeData(String requestUrl) {

        // Simulate long loading time of 2 seconds
        try {
            Thread.sleep(2000);
        ...
        try {
            ...
            Log.v(LOG_TAG, "*************************** This is the jsonRespons from the HTTP request on the URL: " + jsonResponse);
        } catch (IOException e) {
            Log.e(LOG_TAG, "Error closing input stream", e);
        }

        // Extract relevant fields from the JSON response and create an ArrayList of Earthquakes
        ArrayList<Earthquake> earthquakeList = extractFeatureFromJson(jsonResponse);
    ...
    private static ArrayList<Earthquake> extractFeatureFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            // Extract the array of features JSONObjects
            JSONArray earthquakeArray = baseJsonResponse.getJSONArray("features");

            // Loop through the array of features JSONObjects
            for (int i = 0; i < earthquakeArray.length(); i++){
                
                // Get the features JSONObject at position i
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);
                Log.v("QueryUtils", "" + currentEarthquake.toString());

                // Get the properties JSONObject
                JSONObject properties = currentEarthquake.getJSONObject("properties");

                // Extract the magnitude and convert it to a String
                ...
                // Create a new Earthquake with magnitude, location and date
                Earthquake runningEarthquake = new Earthquake(mag, location, timeInMilliseconds, url);

                // Add this Earthquake to the list of earthquakes
                earthquakes.add(runningEarthquake);
                Log.v(LOG_TAG, "Object Nr. " + (i+1) + " / " + earthquakeArray.length() + " finished");
                Log.v(LOG_TAG, "This is Earthquake Nr. " + (i+1) + ": " + runningEarthquake.toString());
